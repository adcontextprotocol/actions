...
import { type ActionInputs, resolveConfig } from './resolve-config.js'
...
async function postBotSkipCommentOnce(params: {
  ...
  await octokit.rest.issues.createComment({
    ...
    issue_number: prNumber,
    body: wrapWithMarker({
      body: 'Ladon does not review bot-authored PRs.',
      marker,
    }),
  })
}
...
async function postMergeConflictCommentOnce(params: {
  ...
  await octokit.rest.issues.createComment({
    ...
    issue_number: prNumber,
    body: wrapWithMarker({
      body: 'Ladon cannot review this PR until merge conflicts are resolved.',
      marker,
    }),
  })
}

async function readLadonMd(): Promise<string | null> {
  const path = join(process.env.GITHUB_WORKSPACE ?? '.', 'LADON.md')
  if (!existsSync(path)) {
    core.info('LADON.md not found at repo root; using defaults.')
    return null
  }
...
async function fetchPriorDecision(params: {
  ...
  const mine = reviews.filter((r) => r.user?.login === ladonBotLogin).reverse()
...
async function main(): Promise<void> {
  ...
  const ladonMdBody = await readLadonMd()
  const ladonMd = parseLadonMd(ladonMdBody)
  const config = resolveConfig({ ladonMd, actionInputs: inputs })

  const ctx = github.context
  // Reviews run under pull_request_target on public repos so fork PRs can
  // access the app token and Anthropic key; same-repo PRs may also arrive as
  // pull_request. Both carry the pull_request payload read below.
  if (
  ...
  if (eventAction === 'synchronize') {
    try {
      ...
      const lastLadon = [...reviews]
        .reverse()
        .find((r) => r.user?.login === ladonBotLogin)
      priorReviewedSha = lastLadon?.commit_id ?? null
    } catch (err) {
      core.warning(
        `Could not read prior Ladon review SHA: ${err instanceof Error ? err.message : String(err)}`,
      )
    }
  ...
  if (!decision.shouldRun) {
    if (decision.skipReason === 'bot-author') {
      ...
      try {
        ...
        try {
          priorOutcome = (JSON.parse(priorDecisionStr) as { outcome?: string })
            .outcome
        } catch {
        ...
        core.warning(
          `Could not check prior approval state for re-approval: ${err instanceof Error ? err.message : String(err)}`,
        )
      }
    }
    core.setOutput('should-run', 'false')
  ...
  const tempDir = await mkdtemp(join(tmpdir(), 'ladon-setup-'))
