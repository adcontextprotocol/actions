...
export function evaluateShortCircuit(
  ctx: ShortCircuitContext,
): ShortCircuitDecision {
  if (ctx.prState === 'closed')
    return { shouldRun: false, skipReason: 'pr-closed' }
  ...
  if (ctx.triggeringActor && ctx.triggeringActor === ctx.ladonBotLogin)
  ...
  if (ctx.eventAction === 'synchronize') {
    ...
    if (
      ctx.deltaFiles.length === 0 &&
      ctx.deltaFilesAfterTrivialFilter.length === 0
    )
