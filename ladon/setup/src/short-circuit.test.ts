...
function ctx(
  ...
  return {
    ...
    ladonBotLogin: 'ladon-the-approver[bot]',
...
describe('evaluateShortCircuit', () => {
  test('open PR with diff → should-run=true', () => {
    expect(evaluateShortCircuit(ctx())).toEqual({
      shouldRun: true,
      skipReason: null,
    })
  })
  ...
  test('bot author → skip with bot-author', () => {
    expect(
      evaluateShortCircuit(ctx({ authorLogin: 'dependabot[bot]' })),
    ).toEqual({
      shouldRun: false,
  ...
  test('pure rebase on synchronize → skip with pure-rebase', () => {
    expect(
      evaluateShortCircuit(
        ctx({ eventAction: 'synchronize', isPureRebase: true }),
      ),
  ...
  test('triggeringActor matches ladonBotLogin → skip with self-triggered', () => {
    expect(
      evaluateShortCircuit(
        ctx({
          triggeringActor: 'ladon-the-approver[bot]',
          ladonBotLogin: 'ladon-the-approver[bot]',
        }),
      ),
  ...
  test('triggeringActor matches but ladonBotLogin empty → still runs', () => {
    expect(
      evaluateShortCircuit(
        ctx({
          triggeringActor: 'ladon-the-approver[bot]',
