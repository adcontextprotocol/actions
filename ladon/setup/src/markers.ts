export function wrapWithMarker(params: {
  body: string
  marker: string
}): string {
  return `${params.body}\n\n<!-- ladon-marker:${params.marker} -->`
}

export function containsMarker(body: string, marker: string): boolean {
  return body.includes(`<!-- ladon-marker:${marker} -->`)
}
...
const DECISION_RE = /<!--\s*ladon-decision:\s*([\s\S]*?)\s*-->/
...
export function parseDecisionMarker(body: string): PriorDecision | null {
  ...
  try {
    ...
    if (
      ...
      ['approve', 'request-changes', 'comment', 'escalate'].includes(
        parsed.outcome,
      ) &&
      typeof parsed.high_risk === 'boolean' &&
      Array.isArray(parsed.reasons)
    ) {
      const findings: PriorFinding[] | undefined = Array.isArray(
        parsed.findings,
      )
