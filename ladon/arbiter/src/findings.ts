...
export function parseFindings(raw: string): FindingsPayload {
  ...
  if (typeof obj.summary !== 'string')
    throw new Error('findings: missing summary')
  if (!Array.isArray(obj.findings))
    throw new Error('findings: findings is not an array')

  const findings: Finding[] = obj.findings.map((f, i) => {
    if (typeof f !== 'object' || f === null)
      throw new Error(`findings[${i}]: not an object`)
    const item = f as Record<string, unknown>
    if (typeof item.severity !== 'string' || !SEVERITIES.has(item.severity)) {
      throw new Error(
        `findings[${i}]: invalid severity ${String(item.severity)}`,
      )
    }
    if (typeof item.category !== 'string' || !CATEGORIES.has(item.category)) {
      throw new Error(
        `findings[${i}]: invalid category ${String(item.category)}`,
      )
    }
    if (typeof item.title !== 'string')
      throw new Error(`findings[${i}]: title missing`)
    if (typeof item.rationale !== 'string')
      throw new Error(`findings[${i}]: rationale missing`)
    if (typeof item.file !== 'string')
      throw new Error(`findings[${i}]: file missing`)
