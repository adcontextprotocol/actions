...
Ladon classifies findings into four severities. Both the reviewer (which assigns severities) and the arbiter (which interprets them for the gate decision) use these definitions verbatim.
...
>
> Why this matters: repos accept PRs from non-technical contributors (consultants, partners, marketing, etc.) whose changes must never auto-merge without a human even if the diff is mechanically clean. The \`## No-Auto-Approve Teams\` LADON.md section is how each repo declares the people whose work needs that extra eye. Treat it as the most important rule in this file.
...
> governance, LADON.md-integrity governance) that this repo's \`## Repo
...
- **escalate** — Requires explicit human reviewers (destructive change, schema migration, sensitive-category problem, etc.). Body lists the escalation reasons. The action will also request the configured escalation reviewers and apply the \`ladon/needs-human-review\` label.
...
4. **Escalation reasons** section (only on \`escalate\`), listing each reason in plain English (e.g. "Deletes \`terraform/prod/main.tf\` — production infra change"). For a \`gated_paths\` escalation (row 2), state the gated file(s) and that human/CODEOWNERS review is required — do not re-explain the gate's rationale at length; that lives in LADON.md, not the PR comment.
