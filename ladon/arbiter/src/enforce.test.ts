...
describe('enforceDecisionGuards', () => {
  ...
  test('does not duplicate the gated-paths reason if the LLM already cited it verbatim', () => {
    ...
    const reason = `This PR touches a path under a hard, non-overridable approval gate (${gatedPathsReasons.join('; ')}) and the current GitHub review decision is 'REVIEW_REQUIRED', not APPROVED. This is a hard gate enforced in code — Ladon cannot auto-approve until a human/CODEOWNERS approval is recorded, regardless of how clean the diff is.`
    const decision = {
      ...baseDecision,
      outcome: 'escalate' as const,
