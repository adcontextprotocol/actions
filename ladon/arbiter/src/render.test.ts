...
describe('renderReviewBody', () => {
  test('approve: clean body with summary', () => {
    ...
    expect(body).toContain('## Ladon verdict: Approve')
    expect(body).toContain('No issues found.')
    expect(body).toContain('<!-- ladon-decision:')
  })
  ...
  test('escalate body lists reasons', () => {
    ...
    expect(body).toContain('## Ladon verdict: Escalate to human review')
  ...
  test('embeds sticky decision marker with parsable JSON', () => {
    ...
    const match = /<!-- ladon-decision:\s*([\s\S]*?)\s*-->/.exec(body)
