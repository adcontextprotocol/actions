...
export function computeNextVersion(params: {
  ...
  const highestMajor = existingVersions.reduce(
    (max, version) => Math.max(max, semver.major(version)),
    0,
  )
  if (declaredMajor.major < highestMajor) {
    throw new Error(
      `declared major ${declaredMajor.major} is below the highest published major ${highestMajor}; refusing to cut a backward version line`,
    )
  }
